#Define i for each product: 1=cancer, 2=pif,
i = 3
##### model meta data ####
...
metrics_gbm <-
  data.frame(train = c(
    Metrics:: accuracy(master$target, master$gbm_pred_num),
  ...
  dev = c(
    Metrics::accuracy(dev$target, dev$gbm_pred_num),
  ...
  test = c(
    Metrics::accuracy(test$target, test$gbm_pred_num),
...
metrics_glm <-
  data.frame(train = c(
    Metrics::accuracy(master$target, master$glm_pred_num),
  ...
  dev = c(
    Metrics::accuracy(dev$target, dev$glm_pred_num),
  ...
  test = c(
    Metrics::accuracy(test$target, test$glm_pred_num),
...
metrics_rf <-
    data.frame(train = c(
      Metrics::accuracy(master$target, master$rf_pred_num),
...
resids_rf  <- DALEX::model_performance(explainer_rf)
resids_gbm <- DALEX::model_performance(explainer_gbm)
...
vip_rf  <-
  variable_importance(explainer_rf, loss_function = loss_root_mean_square, n_sample = 1000)
vip_gbm  <-
  variable_importance(explainer_gbm, loss_function = loss_root_mean_square, n_sample = 1000)
vip_glm  <-
  variable_importance(explainer_glm, loss_function = loss_root_mean_square, n_sample = 1000)
vip_xgb_auc <-
